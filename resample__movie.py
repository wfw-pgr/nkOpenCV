...
def resample__movie( inpFile=None, outFile=None, output_format=None, \
    ...
    if ( resize is not None ):
        if   ( type(resize) in [ float, int, np.float64 ] ):
            resize = [ int(width*resize), int( height*resize ) ]
    ...
    if ( resize is not None ):
        print( "[save__movieFrames.py] original image         :: ( w x h = {} x {} ) "\
               .format( width, height ) )
        print( "[save__movieFrames.py] resized  image         :: ( w x h = {} x {} ) "\
