...
def extract__rectangularRegion( image =None, bb=None   , center_pos=None, \
    ...
    for ik,hbb in enumerate( bb ):
        ...
        if ( ( xfrom < 0 ) or ( yfrom < 0 ) or ( xtill > Lx ) or ( ytill > Ly ) ):
            print( "[extract__rectangularRegion.py] region exceeds image.... " )
            print( "[extract__rectangularRegion.py] need padding ( not yet padding function. ) " )
            # img      = np.ones_like( (ytill-yfrom,xtill-xfrom, 3 ) )
            # img      = np.meshgrid( np.arange() )
            # img      = np.ones_like( (ytill-yfrom,xtill-xfrom, 3 ) )
            # img =
            img = None
        else:
        ...
        #     print( "[extract__rectangularRegion.py] image shape is illegal....  " )
        #     print( "[extract__rectangularRegion.py] img'shape :: {} ".format( img.shape ) )
        #     print( "[extract__rectangularRegion.py] bb        :: {} ".format( hbb       ) )
        #     sys.exit()
        if ( img is not None ):
